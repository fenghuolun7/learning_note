...
public:
  ...
  void register_(const std::shared_ptr<Observer> &x);
  void unregister();
...
void Subject::register_(const std::shared_ptr<Observer> &x)
{
...
void process(const std::shared_ptr<Foo>& sp, const std::shared_ptr<Subject>& subject)
{
  sp->observe(subject);
